...
renderChart =
function (ct, chartId, include_assets = F, ...) 
{
    add_ext_widgets(ct$lib)
    assetHTML <-
        ifelse(include_assets,
               paste(paste(add_lib_assets(ct$lib, ...), collapse="\n"),
                     "\n",
                     add_style_(ct$params$width, ct$params$height), collapse="\n"),
               "")
    chartDiv <-
        render_template(ct$templates$chartDiv,
                        list(chartId=chartId, 
                             lib=ct$LIB$name,
...
getValues =
function(x, y, z=NULL, name=NULL, color=NULL){
...
getTimelapseValues =
function(date, y, z=NULL, name=NULL){
